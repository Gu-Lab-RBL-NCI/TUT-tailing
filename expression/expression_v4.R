...
miRNA_sample <- aggregate(miRNA_summary$TOTAL_READS, by=list(SAMPLE=miRNA_summary$SAMPLE),sum)
colnames(miRNA_sample) <- c("SAMPLE","CPM_miRNA")
miRNA_summary <- merge(miRNA_summary, miRNA_sample, by="SAMPLE") 
miRNA_summary$CPM_miRNA <- miRNA_summary$TOTAL_READS/miRNA_summary$CPM_miRNA*1000000
...
write.table(TUT2_change, "TUT2_change.tsv", append = F, row.names = F, sep = "\t")
...
write.table(TUT4_change, "TUT4_change.tsv", append = F, row.names = F, sep = "\t")
...
write.table(TUT7_change, "TUT7_change.tsv", append = F, row.names = F, sep = "\t")
...
write.table(DKO_change, "DKO_change.tsv", append = F, row.names = F, sep = "\t")
...
write.table(TKO_change, "TKO_change.tsv", append = F, row.names = F, sep = "\t")
...
write.table(all_changes, "venn4.tsv", append = F, row.names = F, sep = "\t")
unique(all_changes$MIRNA)

all$avg

write.table(TUT2model, "model.tsv", append = F, row.names = F, sep = "\t")
