...
all$avgDKO <- (all$DKO+all$DKO2)/2
all$FC_TKO_DKO <- all$TKO/all$avgDKO
all$FC_TKO2_DKO <- all$TKO2/all$avgDKO
all$log2FC_TKO_DKO <- log2((all$FC_TKO_DKO+all$FC_TKO2_DKO)/2)
...
par(mfrow=c(1,1))
datLog2FC <-  as.numeric(monoA_nontemplated$log2FC_TKO_DKO)
monoA_nontemplated_TKO_DKO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(monoA_nontemplated))) 
plot(breaks, monoA_nontemplated_TKO_DKO, type="n", xlab = "mono-adenylated isomiRs fold-change Log2 (KO/WT)", ylab = "Cumulative fraction",ylim=c(0,1), xlim=c(-5,5))
lines(breaks, monoA_nontemplated_TKO_DKO, col="red")

datLog2FC <-  as.numeric(monoA_templated$log2FC_TKO_DKO)
monoA_templated_TKO_DKO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(monoA_templated))) 
lines(breaks, monoA_templated_TKO_DKO, col="black")

datLog2FC <-  as.numeric(canonical$log2FC_TKO_DKO)
canonical_TKO_DKO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(canonical))) 
lines(breaks, canonical_TKO_DKO, col="black")

wilcox.test(monoA_nontemplated$log2FC_TKO_DKO,canonical$log2FC_TKO_DKO)[3]
wilcox.test(monoA_templated$log2FC_TKO_DKO,canonical$log2FC_TKO_DKO)[3]
wilcox.test(monoA_templated$log2FC_TKO_DKO,monoA_nontemplated$log2FC_TKO_DKO)[3]
setwd("~/Desktop/P0 TUT specificty/Qiagen_Dec2020/cumulative-curves-tailing/")
write.table(rbind(breaks, monoA_templated_TKO_DKO, monoA_nontemplated_TKO_DKO,canonical_TKO_DKO), "cumulative-Adenylation-TKO_vs_DKO.txt", sep="\t", append = FALSE)
...
par(mfrow=c(1,1))
datLog2FC <-  as.numeric(monoU_nontemplated$log2FC_TKO_DKO)
monoU_nontemplated_TKO_DKO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(monoU_nontemplated))) 
plot(breaks, monoU_nontemplated_TKO_DKO, type="n", xlab = "mono-uridylated isomiRs fold-change Log2 (TKO/DKO)", ylab = "Cumulative fraction",ylim=c(0,1), xlim=c(-5,5))
lines(breaks, monoU_nontemplated_TKO_DKO, col="red")

datLog2FC <-  as.numeric(monoU_templated$log2FC_TKO_DKO)
monoU_templated_TKO_DKO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(monoU_templated))) 
lines(breaks, monoU_templated_TKO_DKO, col="black")

datLog2FC <-  as.numeric(canonical$log2FC_TKO_DKO)
canonical_TKO_DKO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(canonical))) 
lines(breaks, canonical_TKO_DKO, col="black")

wilcox.test(monoU_nontemplated$log2FC_TKO_DKO,canonical$log2FC_TKO_DKO)[3]
wilcox.test(monoU_templated$log2FC_TKO_DKO,canonical$log2FC_TKO_DKO)[3]
wilcox.test(monoU_templated$log2FC_TKO_DKO,monoU_nontemplated$log2FC_TKO_DKO)[3]
setwd("~/Desktop/P0 TUT specificty/Qiagen_Dec2020/cumulative-curves-tailing/")
write.table(rbind(breaks, monoU_templated_TKO_DKO, monoU_nontemplated_TKO_DKO,canonical_TKO_DKO), "cumulative-Uridylation-TKO_vs_DKO.txt", sep="\t", append = FALSE)

#Guanylation
##TKO_DKO
monoG_nontemplated <- all_nontemplated[which(all_nontemplated$LEN_TRIM==0 & all_nontemplated$SEQ_TAIL=="G"),]
monoG_nontemplated <- monoG_nontemplated[order(-monoG_nontemplated$COUNTS), ]

monoG_templated <- all_templated[which(all_templated$LEN_TRIM==0 & all_templated$SEQ_TAIL=="G"),]
monoG_templated <- monoG_templated[order(-monoG_templated$COUNTS), ]

par(mfrow=c(1,1))
datLog2FC <-  as.numeric(monoG_nontemplated$log2FC_TKO_DKO)
monoG_nontemplated_TKO_DKO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(monoG_nontemplated))) 
plot(breaks, monoG_nontemplated_TKO_DKO, type="n", xlab = "mono-G isomiRs fold-change Log2 (TKO/DKO)", ylab = "Cumulative fraction",ylim=c(0,1), xlim=c(-5,5))
lines(breaks, monoG_nontemplated_TKO_DKO, col="red")

datLog2FC <-  as.numeric(monoG_templated$log2FC_TKO_DKO)
monoG_templated_TKO_DKO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(monoG_templated))) 
lines(breaks, monoG_templated_TKO_DKO, col="black")

datLog2FC <-  as.numeric(canonical$log2FC_TKO_DKO)
canonical_TKO_DKO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(canonical))) 
lines(breaks, canonical_TKO_DKO, col="black")

wilcox.test(monoG_nontemplated$log2FC_TKO_DKO,canonical$log2FC_TKO_DKO)[3]
wilcox.test(monoG_templated$log2FC_TKO_DKO,canonical$log2FC_TKO_DKO)[3]
wilcox.test(monoG_templated$log2FC_TKO_DKO,monoG_nontemplated$log2FC_TKO_DKO)[3]
setwd("~/Desktop/P0 TUT specificty/Qiagen_Dec2020/cumulative-curves-tailing/")
write.table(rbind(breaks, monoG_templated_TKO_DKO, monoG_nontemplated_TKO_DKO,canonical_TKO_DKO), "cumulative-Guanylation-TKO_vs_DKO.txt", sep="\t", append = FALSE)

#C-tail
##TKO_DKO
monoC_nontemplated <- all_nontemplated[which(all_nontemplated$LEN_TRIM==0 & all_nontemplated$SEQ_TAIL=="C"),]
monoC_nontemplated <- monoC_nontemplated[order(-monoC_nontemplated$COUNTS), ]

monoC_templated <- all_templated[which(all_templated$LEN_TRIM==0 & all_templated$SEQ_TAIL=="C"),]
monoC_templated <- monoC_templated[order(-monoC_templated$COUNTS), ]

par(mfrow=c(1,1))
datLog2FC <-  as.numeric(monoC_nontemplated$log2FC_TKO_DKO)
monoC_nontemplated_TKO_DKO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(monoC_nontemplated))) 
plot(breaks, monoC_nontemplated_TKO_DKO, type="n", xlab = "mono-C isomiRs fold-change Log2 (TKO/DKO)", ylab = "Cumulative fraction",ylim=c(0,1), xlim=c(-5,5))
lines(breaks, monoC_nontemplated_TKO_DKO, col="red")

datLog2FC <-  as.numeric(monoC_templated$log2FC_TKO_DKO)
monoC_templated_TKO_DKO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(monoC_templated))) 
lines(breaks, monoC_templated_TKO_DKO, col="black")

datLog2FC <-  as.numeric(canonical$log2FC_TKO_DKO)
canonical_TKO_DKO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(canonical))) 
lines(breaks, canonical_TKO_DKO, col="black")

wilcox.test(monoC_nontemplated$log2FC_TKO_DKO,canonical$log2FC_TKO_DKO)[3]
wilcox.test(monoC_templated$log2FC_TKO_DKO,canonical$log2FC_TKO_DKO)[3]
wilcox.test(monoC_templated$log2FC_TKO_DKO,monoC_nontemplated$log2FC_TKO_DKO)[3]
setwd("~/Desktop/P0 TUT specificty/Qiagen_Dec2020/cumulative-curves-tailing/")
write.table(rbind(breaks, monoC_templated_TKO_DKO, monoC_nontemplated_TKO_DKO,canonical_TKO_DKO), "cumulative-Cytidylation-TKO_vs_DKO.txt", sep="\t", append = FALSE)
...
par(mfrow=c(1,1))
datLog2FC <-  as.numeric(monoG_nontemplated$log2FC_TUT2)
monoG_nontemplated_TUT2KO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(monoG_nontemplated))) 
plot(breaks, monoG_nontemplated_TUT2KO, type="n", xlab = "mono-G isomiRs fold-change Log2 (KO/WT)", ylab = "Cumulative fraction",ylim=c(0,1), xlim=c(-5,5))
lines(breaks, monoG_nontemplated_TUT2KO, col="red")

datLog2FC <-  as.numeric(monoG_templated$log2FC_TUT2)
monoG_templated_TUT2KO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(monoG_templated))) 
lines(breaks, monoG_templated_TUT2KO, col="black")

datLog2FC <-  as.numeric(canonical$log2FC_TUT2)
canonical_TUT2KO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(canonical))) 
lines(breaks, canonical_TUT2KO, col="black")

wilcox.test(monoG_nontemplated$log2FC_TUT2,canonical$log2FC_TUT2)[3]
wilcox.test(monoG_templated$log2FC_TUT2,canonical$log2FC_TUT2)[3]
wilcox.test(monoG_templated$log2FC_TUT2,monoG_nontemplated$log2FC_TUT2)[3]
setwd("~/Desktop/P0 TUT specificty/Qiagen_Dec2020/cumulative-curves-tailing/")
write.table(rbind(breaks, monoG_templated_TUT2KO, monoG_nontemplated_TUT2KO,canonical_TUT2KO), "cumulative-Guanylation-TU2KO.txt", sep="\t", append = FALSE)
...
par(mfrow=c(1,1))
datLog2FC <-  as.numeric(monoC_nontemplated$log2FC_TUT2)
monoC_nontemplated_TUT2KO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(monoC_nontemplated))) 
plot(breaks, monoC_nontemplated_TUT2KO, type="n", xlab = "mono-C isomiRs fold-change Log2 (KO/WT)", ylab = "Cumulative fraction",ylim=c(0,1), xlim=c(-5,5))
lines(breaks, monoC_nontemplated_TUT2KO, col="red")

datLog2FC <-  as.numeric(monoC_templated$log2FC_TUT2)
monoC_templated_TUT2KO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(monoC_templated))) 
lines(breaks, monoC_templated_TUT2KO, col="black")

datLog2FC <-  as.numeric(canonical$log2FC_TUT2)
canonical_TUT2KO = c(0, cumsum(table(cut(datLog2FC, breaks, right=FALSE))/nrow(canonical))) 
lines(breaks, canonical_TUT2KO, col="black")

wilcox.test(monoC_nontemplated$log2FC_TUT2,canonical$log2FC_TUT2)[3]
wilcox.test(monoC_templated$log2FC_TUT2,canonical$log2FC_TUT2)[3]
wilcox.test(monoC_templated$log2FC_TUT2,monoC_nontemplated$log2FC_TUT2)[3]
setwd("~/Desktop/P0 TUT specificty/Qiagen_Dec2020/cumulative-curves-tailing/")
write.table(rbind(breaks, monoC_templated_TUT2KO, monoC_nontemplated_TUT2KO,canonical_TUT2KO), "cumulative-cytidylation-TU2KO.txt", sep="\t", append = FALSE)
