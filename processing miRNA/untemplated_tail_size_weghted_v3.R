...
iso_small <- iso_summary[iso_summary$MIRNA %in% anytail2$MIRNA,]
agg_sample_miRNA <- aggregate(iso_small$READS, by=list(SAMPLE=iso_small$SAMPLE),sum)

Atail <- anytail2[which(anytail2$pureA==T),]
Atail <- aggregate(Atail$READS, by=list(SAMPLE=Atail$SAMPLE,LEN_TAIL=Atail$LEN_TAIL),sum)
Utail <- anytail2[which(anytail2$pureU==T),]
Utail <- aggregate(Utail$READS, by=list(SAMPLE=Utail$SAMPLE, LEN_TAIL=Utail$LEN_TAIL),sum)
Mtail <- anytail2[which(anytail2$pureU==F & anytail2$pureA==F),]
Mtail <- aggregate(Mtail$READS, by=list(SAMPLE=Mtail$SAMPLE,LEN_TAIL=Mtail$LEN_TAIL),sum)

Atail <- merge(Atail,agg_sample_miRNA,by=c("SAMPLE"))
Utail <- merge(Utail,agg_sample_miRNA,by=c("SAMPLE"))
Mtail <- merge(Mtail,agg_sample_miRNA,by=c("SAMPLE"))
...
Atail_mean <- aggregate(Atail$percent_len_tail, by=list(LEN_TAIL=Atail$LEN_TAIL),mean)
colnames(Atail_mean) <- c("LEN_TAIL","Atail_mean")
Atail_sd <- aggregate(Atail$percent_len_tail, by=list(LEN_TAIL=Atail$LEN_TAIL),sd)
colnames(Atail_sd) <- c("LEN_TAIL","Atail_sd")

Utail_mean <- aggregate(Utail$percent_len_tail, by=list(LEN_TAIL=Utail$LEN_TAIL),mean)
colnames(Utail_mean) <- c("LEN_TAIL","Utail_mean")
Utail_sd <- aggregate(Utail$percent_len_tail, by=list(LEN_TAIL=Utail$LEN_TAIL),sd)
colnames(Utail_sd) <- c("LEN_TAIL","Utail_sd")

Mtail_mean <- aggregate(Mtail$percent_len_tail, by=list(LEN_TAIL=Mtail$LEN_TAIL),mean)
colnames(Mtail_mean) <- c("LEN_TAIL","Mtail_mean")
Mtail_sd <- aggregate(Mtail$percent_len_tail, by=list(LEN_TAIL=Mtail$LEN_TAIL),sd)
...
sum_sum <- sum(anytail5$Atail_mean)+sum(anytail5$Utail_mean)+sum(anytail5$Mtail_mean)
anytail5$Atail_mean <- anytail5$Atail_mean/sum_sum*100
anytail5$Utail_mean <- anytail5$Utail_mean/sum_sum*100
anytail5$Mtail_mean <- anytail5$Mtail_mean/sum_sum*100
sum(anytail5$Atail_mean)+sum(anytail5$Utail_mean)+sum(anytail5$Mtail_mean)
anytail5$Atail_sd <- anytail5$Atail_sd/sum_sum*100
anytail5$Utail_sd <- anytail5$Utail_sd/sum_sum*100
anytail5$Mtail_sd <- anytail5$Mtail_sd/sum_sum*100

write.table(anytail5, "wtail_by_length_composition_3p.txt", sep="\t", append = FALSE, row.names = F)
